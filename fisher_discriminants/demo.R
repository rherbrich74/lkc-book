...
book <- function (output='SCREEN') {
  ...
  if (output == 'PS') {
    postscript ("FD_generative.ps", horizontal=FALSE);
  }
  ...
  contour (x, y, z.minus, levels=c (0.00272), drawlabels=FALSE,
           lwd=1, lty=2, add=TRUE);
  if (output == 'PS') {
    cat ("[Fisher Discriminant generative plot generated]\n");
  } else {
  ...
  if (output == 'PS') {
    postscript ("FD_projections.ps", horizontal=FALSE);
  }
  ...
  if (output == 'PS') {
    cat ("[Fisher Discriminant data distribution plot generated]\n");
  } else {
    readline("Press any key for next plot");
  }
  dev.off ();
