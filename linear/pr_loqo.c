 ...
 *
 * Author:      Alex J. Smola
 * Modified:    Ralf Herbrich (adapted to R)
 * Created:     10/14/97
 * Updated:     11/08/97
 * Updated:     24/04/99
 *
	    ...
	    double margin, double bound, int restart) 
{
  ...
  while (status == STILL_RUNNING) {
    ...
    sigfig = log10(ABS(primal_obj) + 1.0) -
	...
	alfa = min(alfa, delta_z[i]/z[i]);
      ...
      alfa = (margin - 1.0) / alfa;
      
      /* compute mu */
      for (i=0, mu=0; i<n; i++)
	mu += z[i] * g[i] + s[i] * t[i];
      mu = mu / (2.0* (double) n);
      mu = mu * sqr((alfa - 1) / (alfa + 10.0));
...
void pr_loqo_R (int *n, int *m, double *c, double *h_x, double *a, double *b,
		double *l, double *u, double *primal, double *dual, 
		int *verb, double *sigfig_max, int *counter_max, double *margin,
		double *bound, int *restart)
{
  switch (pr_loqo (*n, *m, c, h_x, a, b, l, u, primal, dual, *verb,
		   *sigfig_max, *counter_max, *margin, *bound, *restart)) {
  case SUBOPTIMAL_SOLUTION: printf ("suboptimal solution found!\n"); break;
  case ITERATION_LIMIT    : printf ("iteration limit reached!\n"); break;
  case PRIMAL_INFEASIBLE  : printf ("primal problem is infeasible!\n"); break;
  case DUAL_INFEASIBLE    : printf ("dual problem is infeasible!\n"); break;
  case PRIMAL_AND_DUAL_INFEASIBLE: printf ("primal and dual are infeasible!\n"); break;
  case INCONSISTENT       : printf ("inconsistency error!\n"); break;
  case PRIMAL_UNBOUNDED   : printf ("primal problem unbounded!\n"); break;
  case DUAL_UNBOUNDED     : printf ("dual problem unbounded!\n"); break;
  case TIME_LIMIT         : printf ("time limit reached!\n"); break;
  }
  return;
}
